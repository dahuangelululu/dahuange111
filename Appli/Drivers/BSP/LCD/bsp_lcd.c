...
/*================= 新增：在本文件里分配一块帧缓冲 =================*/
static uint16_t lcd_framebuffer[BSP_LCD_WIDTH * BSP_LCD_HEIGHT];
...
/* 对外提供一个获取 framebuffer 地址的接口（可选用） */
uint16_t *bsp_lcd_get_framebuffer(void)
{
    return lcd_framebuffer;
}

/* 对外提供一个简单清屏接口，颜色用 RGB565 */
void bsp_lcd_clear(uint16_t color)
{
    uint32_t address = hltdc.LayerCfg[bsp_lcd.active_layer].FBStartAdress;
    bsp_lcd_fill_buffer((uint32_t *)address,
                        bsp_lcd.x_size,
                        bsp_lcd.y_size,
                        0,
                        color);
...
void bsp_lcd_init_ex(uint32_t pixel_format, uint32_t width, uint32_t height)
{
    ...
    /* 3. 配置 Layer0 使用这块 framebuffer，窗口覆盖整屏 */
    bsp_lcd_layer_config_t layer_cfg;
    layer_cfg.x0 = 0;
    layer_cfg.y0 = 0;
    layer_cfg.x1 = width;
    layer_cfg.y1 = height;
